                        ...
                        <div className="flex flex-col p-4">
                            {
                                providers && providers.map((provider) => {
                                    if (provider === "discord"){
                                        return <button key="discord" onClick={() => signIn("discord")} className="w-full flex flex-row justify-between items-center group/discord border border-[#7289da] text-[#7289da] hover:text-base-200 hover:bg-[#7289da] p-2 transition-colors">
                                    ...
                                    if (provider === "github"){
                                        return <button key="github" onClick={() => signIn("github")} className="w-full flex flex-row justify-between items-center group/github border border-[#7289da] text-[#7289da] hover:text-base-200 hover:bg-[#7289da] p-2 transition-colors">
                                            <p className="font-bold text-lg">GitHub</p>
                                            <div className="swap group-hover/github:swap-active">
                                                <svg fill="none" className="swap-on" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"> <path d="M21 11V3h-8v2h4v2h-2v2h-2v2h-2v2H9v2h2v-2h2v-2h2V9h2V7h2v4h2zM11 5H3v16h16v-8h-2v6H5V7h6V5z" fill="currentColor"/> </svg>
                                                <svg className="swap-off" height={16} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"> <path fill="currentColor" d="M5 2h4v2H7v2H5V2Zm0 10H3V6h2v6Zm2 2H5v-2h2v2Zm2 2v-2H7v2H3v-2H1v2h2v2h4v4h2v-4h2v-2H9Zm0 0v2H7v-2h2Zm6-12v2H9V4h6Zm4 2h-2V4h-2V2h4v4Zm0 6V6h2v6h-2Zm-2 2v-2h2v2h-2Zm-2 2v-2h2v2h-2Zm0 2h-2v-2h2v2Zm0 0h2v4h-2v-4Z"/> </svg>
                                            </div>
                                        </button>;
                                    }
                                })
