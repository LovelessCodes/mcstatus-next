...
const AddRoute = () => {
    ...
    const { mutate: addNewRoute, isLoading: isInserting } =
        api.route.insertRoute.useMutation({
            onSuccess: async () => {
                ...
                <label className="modal-box rounded-none relative" htmlFor="">
                    <div className="flex flex-col p-10">
                        ...
                        <div className="form-control">
                            <div
                                className="tooltip"
                                data-tip="You need to own the domain, and have the domain or subdomain pointed at the server running the router."
                            >
                                <label className="flex flex-col text-left">
                                    ...
                                    <span className="label-description text-xs italic opacity-75">
                                        The address you want the server to be
                                        accessible on.
                                    </span>
                                </label>
                            </div>
                            <input
                                type="text"
                                minLength={1}
                                placeholder="mc.test.com"
                                className={`input ${
                                    (!url.safeParse(address).success ||
                                        address.length === 0) &&
                                    'border-pink-600'
                        ...
                        <div className="form-control">
                            <div
                                className="tooltip"
                                data-tip="Alternatively use the ip-address of the docker container + the port exposed to the server."
                            >
                                <label className="flex flex-col text-left">
                                    <span className="label-text">
                                        Backend *
                                    ...
                                    <span className="label-description text-xs italic opacity-75">
                                        The docker container&apos;s name + the
                                        port exposed to the server.
                                    </span>
                                </label>
                            </div>
                            <input
                                type="text"
                                minLength={1}
                                placeholder="docker_container:25565"
                                className={`input ${
                                    (!back.safeParse(backend).success ||
                                        backend.length === 0) &&
                                    'border-pink-600'
                                } input-bordered rounded-none`}
                                value={backend}
                        ...
                        <div className="w-full btn-group">
                            {isInserting ? (
                                <button className="btn btn-primary rounded-none btn-disabled">
                                    Adding...
                                </button>
                            ) : (
                                <button
