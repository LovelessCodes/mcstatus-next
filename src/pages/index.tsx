...
import CreateRoute from "~/components/modals/addRoute";
...
const Home: NextPage = () => {
  ...
  if (isRoutesLoading || !Array.isArray(routes)) return (
    <>
      <Head>
      ...
      <main className="h-screen w-screen justify-center flex flex-col items-center overflow-hidden">
      ...
      <main className="h-screen w-screen justify-center flex flex-col items-between overflow-hidden">
        <div className="stats rounded-none bg-gradient-to-br from-primary to-base-100 text-primary-content lg:h-[10%] h-[15%]">
          <div className="stat">
            ...
            <div className="stat-value text-slate-200">{routes?.length}</div>
          </div>
          <div className="stat">
            <div className="stat-title text-slate-200">Active Servers</div>
            <div className="stat-value text-slate-200">{servers.filter((s: any) => s.online).length}</div>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="overflow-x-auto w-full">
                <table className="table table-sm">
                  <tbody>
                    {routes?.map((route: { address: string }, i: number) => {
              ...
              return (
                <div key={i} className="card rounded-none bg-base-200 w-full h-1/6 flex flex-row">
                  <div className="ml-4 h-[64px] self-center">
                    <Image className="bg-base-200" width={64} height={64} alt={server.host} src={server.icon ? server.icon : '/images/server-icon.webp'}/>
                  </div>
